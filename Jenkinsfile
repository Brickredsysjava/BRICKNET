                ...
                script {
                    ...
                    services.each { service ->
                        dir(service) {
                            ...
                            // Copy the built JAR to the remote server
                            sh "scp -i id_rsa target/$service.jar root@192.168.0.9:~/$service/"

                            // Remove existing Docker containers and images
                            sh "ssh root@192.168.0.9 'docker service rm $service || true'"
                            sh "ssh root@192.168.0.9 'docker rmi kabiraneja/ubuntu-java-maven || true'"
                        }
                    }

                    // Copy the updated Docker Compose file to the remote server and deploy services
                    sh "scp -i id_rsa docker-compose.yml root@192.168.0.9:~/"
                    sh "ssh root@192.168.0.9 'cd ~ && docker stack deploy --compose-file docker-compose.yml stack_name'"
                }
            }
        }
    }
