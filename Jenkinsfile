    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                checkout scm
                sh " ssh root@172.16.1.9 'docker stack rm bricknetstack || true'"
                sh " ssh root@172.16.1.9 ' ./remove_service.sh '"
                sh " ssh root@172.16.1.9 ' docker network rm bricknetstack_default || true'"
            }
        }
         ...
         stage('Build Eureka-Server') {
            steps {
                ...
                sh ' scp -i id_rsa /var/jenkins_home/workspace/bricknet/Eureka-Server/target/eureka.jar root@172.16.1.9:~/eureka/'
        ...
        stage('Build api-gateway') {
            steps {
                ...
                sh 'cd api-gateway && mvn clean package -DskipTests'
                 ...
                 sh "ssh root@172.16.1.9 'cd /root'"
                 sh "ssh root@172.16.1.9 'rm -rf api-gateway || true'"
                 sh "ssh root@172.16.1.9 'mkdir api-gateway'"

                sh ' scp -i id_rsa /var/jenkins_home/workspace/bricknet/api-gateway/target/api-gateway.jar root@172.16.1.9:~/api-gateway/'

                sh "ssh root@172.16.1.9 'docker stop root_api-gateway_1 || true'"
                sh "ssh root@172.16.1.9 'docker rm root_api-gateway_1 || true'"
                sh "ssh root@172.16.1.9 'docker rmi root_api-gateway_1 ||true'"
        ...
        stage('Build auth-server') {
            steps {
                ...
                sh ' scp -i id_rsa /var/jenkins_home/workspace/bricknet/auth-server/target/auth-server.jar root@172.16.1.9:~/auth-server/'
        ...
        stage('Build SuperAdmin') {
            steps {
                ...
                sh ' scp -i id_rsa /var/jenkins_home/workspace/bricknet/SuperAdmin/target/superadmin.jar root@172.16.1.9:~/superadmin/'
        ...
        stage('Build notification') {
                    steps {
                        ...
                        sh "ssh root@172.16.1.9 'cd /root'"
                        sh "ssh root@172.16.1.9 'rm -rf notification || true'"
                        sh "ssh root@172.16.1.9 'mkdir notification'"

                        sh ' scp -i id_rsa /var/jenkins_home/workspace/bricknet/notification/target/notification.jar root@172.16.1.9:~/notification/'

                        sh "ssh root@172.16.1.9 'docker stop root_notification_1 || true'"
                        sh "ssh root@172.16.1.9 'docker rm root_notification_1 || true'"
                        sh "ssh root@172.16.1.9 'docker rmi root_notification_1 ||true'"
                    }
                }
        ...
        stage('Build community') {
                            steps {
                                ...
                                sh ' scp -i id_rsa /var/jenkins_home/workspace/bricknet/community/target/community.jar root@172.16.1.9:~/community/'
         ...
         stage('Build suggestion') {
                                     steps {
                                         ...
                                         sh ' scp -i id_rsa /var/jenkins_home/workspace/bricknet/suggestion/target/suggestion.jar root@172.16.1.9:~/suggestion/'
         ...
         stage('Build taskcenter') {
                                     steps {
                                         ...
                                         sh ' scp -i id_rsa /var/jenkins_home/workspace/bricknet/taskcenter/target/taskcenter.jar root@172.16.1.9:~/taskcenter/'
         ...
         stage('Build broadcast') {
                                     steps {
                                         ...
                                         sh ' scp -i id_rsa /var/jenkins_home/workspace/bricknet/broadcast/target/broadcast.jar root@172.16.1.9:~/broadcast/'
         ...
         stage('Build media-service') {
                                     steps {
                                         ...
                                         sh ' scp -i id_rsa /var/jenkins_home/workspace/bricknet/media-service/target/mediaservice.jar root@172.16.1.9:~/mediaservice/'
         ...
         stage('Build attendance') {
                                     steps {
                                         ...
                                         sh ' scp -i id_rsa /var/jenkins_home/workspace/bricknet/attendance/target/attendance.jar root@172.16.1.9:~/attendance/'
        ...
        stage('Deploy All Microservices') { 
            steps {
                sh "ssh root@172.16.1.9 'cd /root'"
                sh "ssh root@172.16.1.9 'rm -rf docker-compose.yml || true'"
                sh ' scp -i id_rsa /var/jenkins_home/workspace/bricknet/docker-compose.yml root@172.16.1.9:~/'
                // sh " ssh root@172.16.1.9 'docker-compose up -d'"
                sh " ssh root@172.16.1.9 'docker stack deploy -c docker-compose.yml bricknetstack'"
            }
        }
