    ...
    stages {
        stage('Checkout') {
         ...
         stage('Build Eureka') {
            steps {
                ...
                sh 'cd Eureka && mvn clean package -DskipTests'
            }
        }
        ...
        stage('Deploy Eureka') {
            steps {
                ...
                sh "ssh root@192.168.1.9 'rm -rf EurekaServer || true'"
                sh "ssh root@192.168.1.9 'mkdir EurekaServer'"
                
                sh ' scp -i id_rsa /var/jenkins_home/workspace/bricknet/EurekaServer/target/EurekaServer.jar root@192.168.1.9:~/EurekaServer/'
                sh ' scp -i id_rsa /var/jenkins_home/workspace/bricknet/EurekaServer/Dockerfile root@192.168.1.9:~/EurekaServer/'
                ...
                sh "ssh root@192.168.1.9 'docker build -t EurekaServer /root/test-1'"
